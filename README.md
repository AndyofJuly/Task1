# Task1
## 至2020.7.23日14:30，代码更新说明
对代码结构进行了一些调整和优化

> 1.成就系统中使用枚举类来代替excel配置，便于开发中理解id与描述的对应关系

> 2.背包服务模块使用饿汉式的单例模式，对外提供一个调用服务的对象。

> 3.修改了公会id和装备唯一id的生成方式

> 4.将Netty的I/O线程与业务线程分离开来，使用自定义的业务线程来处理游戏业务

> 5.修复了一些方法中的bug

---
## 至2020.7.21日14:30，代码更新说明
对代码结构进行了一些调整和优化

> 1.将角色模块中的方法按所属拆分到了场景、背包、社交等业务模块中

> 2.对成就系统进行了调整，修改为每个事件作为一个Subject，因此如果一个Service类中包含多个成就事件，则需要构造多个Subject，每个Subject对应地注册一个观察者

> 3.增加了成就的进度条属性，并相应增加了数据库相关的操作。

---
## 至2020.7.20日21:30，代码更新说明
修复了一些bug，对一些问题进行了修正(此次commit注释时有一些问题)

> 1.检查代码时发现之前从背包中取武器的方式不对，因此进行了修正，装备新增唯一id属性

> 2.将很多实体类的属性修饰由int修改为Integer，因为本项目中使用包装类型更加安全和规范

> 3.使用@Autowired完成自动注入(之前由于没有深入理解自定义反射，因而没有修改成功)

> 4.对装备由手上放回背包时没有减去攻击力的bug进行了修复

> 5.对成就系统的实现进行了一些调整，事件源以集合的形式来注册观察者，提高了扩展性

---
## 至2020.7.18日11:20，代码更新说明
新增了一些功能，修复了一些bug，对代码进行了一些调整

> 1.增加了一些注释和说明，使得代码看起来更整洁了

> 2.血量、物品数量等超出上下限的bug进行了修复；背包物品存放不下时没有处理的bug

> 3.角色被pk时进行通知、角色加入队伍和开启副本时进行通知、挑战退出时的原因通知、入会申请消息通知

> 4.群体技能进行了一些调整；群体攻击修改为对视野范围内的怪物进行攻击，群体治疗修改为对队友进行治疗

> 5.BOSS增加自由移动和普通攻击的简单AI

---
## 至2020.7.15日18:00，代码更新说明
完善补充了许多遗漏功能，对部分方法进行了一些调整和优化

> 1.用户登录只能选择自己已有角色登录-通过在客户端记录登录的用户信息并查找角色来实现

> 2.同场景生成多(2)只同种怪物，之前每种怪物都只生成了一只

> 3.打败怪物后对同场景玩家进行通知-通过在服务端记录每个角色和对应的通信端口来实现

> 4.对npc进行实体化，并且能够自由移动。由于怪物和npc的移动，会跨越场景中的小格子，角色视野会造成变化，对此进行了实现

> 5.组队时队友掉线，会对其他队友进行通知；玩家一口价出售物品成功后会进行通知；角色竞价开始时对场景中其他角色进行通知，某个角色喊价后对其他角色进行通知，竞价结束时通知竞价结束

> 6.角色掉线时自动保存数据到数据库

> 7.面对面交易、一口价交易可以交易多件物品

> 8.对方法体较长的方法进行了适当改写和调整

后续计划进行测试和优化。目前代码仍然有很大的优化和改进区间，部分功能的实现还可以再完善。
