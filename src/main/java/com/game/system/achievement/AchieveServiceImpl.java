...
public class AchieveServiceImpl implements IAchieveService {
    ...
    private void ifSlayPartiMonster(int monsterId,Role role){
        for(Integer achievId : AchieveResource.getAchieveStaticHashMap().keySet()){
            if(Const.achieve.TASK_MONSTER.equals(AchieveResource.getAchieveStaticHashMap().get(achievId).getDesc())){
...
/*    public void talkNpc(MyEvent event){
    ...
    //添加一个好友-两个人都判断 todo 申请消息应该发送给对方客户端
    @Override
    ...
    public void ifFirstJoinTeam(Role role){
        firstAchieve(Const.achieve.TASK_FIRST_TEAM,role);
    }

    //第一次加入公会
    @Override
    public void ifFirstJoinUnion(Role role){
        firstAchieve(Const.achieve.TASK_FIRST_UNION,role);
    }

    //第一次与玩家交易
    @Override
    public void ifFirstTradeWithPlayer(Role role){
        firstAchieve(Const.achieve.TASK_FRIST_TRADE,role);
    }

    //第一次在pk中战胜
    @Override
    public void ifFirstPkSuccess(Role role){
        firstAchieve(Const.achieve.TASK_PK_SUCCESS,role);
    }
