...
import com.game.service.impl.RoleServiceImpl;
...
public class RoleController {
    ...
    public ResponseInf registerMesseage() {
        if(iRoleDao.insertRegister(strList.get(1),strList.get(2))){
    ...
    public ResponseInf loginMesseage() {
        ...
        int userId = iRoleDao.selectLogin(strList.get(1),strList.get(2));
        if(userId!=0){
            //获得该用户的所有角色信息
            String roleMsg = "，你目前的角色有：";
            ArrayList<Integer> roleList = iRoleDao.selectRole(userId);
            for(Integer roleId : roleList){
                roleMsg += roleId+" ";
            }
            return ResponseInf.setResponse(Const.start.ULOGIN_SUCCESS+roleMsg,getRole());
        }else {
            return ResponseInf.setResponse(Const.start.ULOGIN_FAILURE,getRole());
        }
    }
